...
def _make_altyn_registry_command_handler(manager_key: str):
    ...
    (по прямой просьбе владелицы, часть 32, см. config.ALTYN_MANAGER_COMMANDS/
    altyn_registry.py) — статусы по банкам Алтына по конкретному менеджеру, живьём из
    гугл-таблиц. Только в личке, только для владелицы — тот же паттерн гейтинга, что у
    остальных персональных команд."""
    ...
    async def handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            logger.exception("Не удалось получить реестры банков Алтына для менеджера %s", manager_key)
            await update.message.reply_text(
                f"Не смогла получить реестры банков Алтына для «{label}» — попробую в следующий раз."
            )
            return
