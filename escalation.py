...
по-своему.
Если ниже приложена "Память чата" — это сжатая сводка более ранней переписки в этом же
групповом чате (см. chat_memory.py), автоматически накопленная ботом. Используй её для
контекста (кто чем занимается в этом чате, какие решения/договорённости уже были, какие
вопросы оставались открытыми), но помни: она может быть неполной или не самой свежей
(бот не видит сообщений, отправленных до того как его добавили в этот чат), и это не
факт, что она относится именно к текущему вопросу — не выдумывай на её основе деталей,
которых там прямо нет.
...
def draft_initial_answer(
    ...
    kb.detect_countries), если чат закреплён за проектом (project_key) — список открытых
    задач этого проекта из ClickUp, и если для этого чата уже накоплена "память" (см.
    chat_memory.get_summary_for_draft, по прямой просьбе владелицы 06.09) — сжатую сводку
    более ранней переписки этого чата. Всё это — для контекста, не как источник истины.
    ClickUp/память читаются лучшим усилием: если не получилось — просто продолжаем без
    этого контекста, не роняем весь черновик. Пустая строка, если Claude не вернул текст —
    вызывающий код в этом случае переходит сразу к "напиши сама"."""
    ...
    memory_context = f"\n\nПамять чата (см. систему выше):\n{chat_summary}" if chat_summary else ""
    user_content = (
        f"Групповой чат: «{group_title}»\nВопрос от {asker_name}: {question}{tasks_context}{memory_context}"
    )
    response = client.messages.create(
