...
# созданной задаче (см. _resolve_assignee_id в bot.py). Осознанно НЕ используем нечёткое
# сравнение и короткие неоднозначные прозвища (например "Коля"/"Ник" могут быть и
# Николаем Хребетом, и Ником Галтом) — только полные имена/фамилии, чтобы не назначить
# задачу не тому человеку. Если имени нет в словаре — Assignee просто не проставляется,
# в заголовке задачи имя всё равно останется (см. task_extractor.py). Ключи — в нижнем
# регистре, сравнение регистронезависимое.
