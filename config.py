...
# сравнение — только конкретные имена/прозвища, которые реально использует команда, чтобы
# не назначить задачу не тому человеку. Если имени нет в словаре — Assignee просто не
# проставляется, в заголовке задачи имя всё равно останется (см. task_extractor.py).
# Ключи — в нижнем регистре, сравнение регистронезависимое.
