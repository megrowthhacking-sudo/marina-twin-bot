...
# Реестры банков Алтына (по прямой просьбе владелицы, часть 32) — 2 гугл-таблицы, обе
# расшарены "по ссылке — все могут читать" (см. altyn_registry.py — читаются напрямую
# публичным CSV-экспортом, без сервисного аккаунта/токена). Ключи верхнего уровня — просто
# для читаемости, реального значения не имеют, кроме file_id/sheets. "columns" — индексы
# нужных колонок (0-based, как в исходном листе): "bank"/"manager" обязательны, "status" и
# "extra" (доп. колонка — метод/дата/агент) опциональны.
ALTYN_REGISTRY_SHEETS = {
    "rs_broker": {
        # "Р/С для А Брокер" — https://docs.google.com/spreadsheets/d/10xe6RsRUL3rArogufKrJtzBB5-I7YEcxUWQUE_QZd1U
        "file_id": "10xe6RsRUL3rArogufKrJtzBB5-I7YEcxUWQUE_QZd1U",
        "sheets": [
            {
                # Это то, что владелица называет "Брокер" — банки Алтын Брокера,
            ...
            {
                # Это "Кошелек Алтын" — банки под Кошелёк Алтын, подключение методов
                # СБП/МИР/ОСТ и т.д.
    ...
    "banki_rf": {
        # "Банки РФ в работе" — https://docs.google.com/spreadsheets/d/1_zvu3BsA7ElW0tFiy396cqaqyYro-MV1
        # Более широкая "холодная" база банков РФ — берём только листы со стабильной
        # колонкой "Менеджер" и реальным статусом по банку (см. altyn_registry.py
        # докстринг — почему остальные листы этой книги не выгружаются).
...
# Персональные команды по менеджерам банковских реестров Алтына (по прямой просьбе
# владелицы, часть 32: "Мне нужны команды для бота, по которым ты ходишь в реестры
# Алтына и выгружаешь статусы по банкам по каждому менеджеру отдельно"). Роспись менеджеров
# сверена по факту с реальными данными в ALTYN_REGISTRY_SHEETS (не весь список,
# изначально названный владелицей — см. проектный статус-док, часть 32): Марина/Саша/
# Рома/Эдуард ни разу не встретились как активный менеджер ни в одном листе, Даниил
# исключён отдельно (по словам владелицы — ушёл в другой отдел, шлюзами больше не
# занимается). "Илья" в реестрах может быть записан как "Илья" или "Илья А" — это один
# и тот же человек, Илья Агафонов, ОТДЕЛЬНЫЙ от Ильи Евдокимова (см. EMPLOYEE_COMMANDS
# ["ilya"] выше — это разные люди с одним именем, разные ClickUp-аккаунты). "match" —
# список префиксов (без учёта регистра), по которым ищем совпадение в колонке
# "Менеджер" (см. altyn_registry.py::_manager_matches) — учитывает варианты написания
# вроде "Лена"/"Лена Ч", "Слава"/"Слава А".
